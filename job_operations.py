...
# ML imports for TF-IDF + Cosine Similarity skill matching
...
# ─────────────────────────────────────────────────────────────────────────────
# ML-POWERED MATCHING  (Algorithm: TF-IDF Vectorization + Cosine Similarity)
# ─────────────────────────────────────────────────────────────────────────────
#
# HOW IT WORKS:
#   1. TfidfVectorizer converts each skill-set string into a numeric TF-IDF
#      vector where each dimension represents a skill token weighted by how
#      rare / important it is across the corpus.
#   2. cosine_similarity() measures the angle between the two vectors.
#      A score of 1.0 = perfect match, 0.0 = no overlap.
#   3. This approach handles partial / fuzzy matches far better than simple
#      string intersection: e.g. "machine learning" and "ml" contribute to
#      the same TF-IDF dimension after tokenisation.
#   4. Experience is scored separately and blended: 70% skill-cosine + 30% exp.
# ─────────────────────────────────────────────────────────────────────────────

def _tfidf_skill_similarity(user_skills_str: str, job_skills_str: str) -> float:
    """
    Compute cosine similarity between two skill strings using TF-IDF vectors.
    Returns a float in [0.0, 1.0].
    """
    # Need at least two documents for TfidfVectorizer
    corpus = [user_skills_str, job_skills_str]
    try:
        vectorizer = TfidfVectorizer(
            analyzer='word',
            token_pattern=r"[a-zA-Z0-9#+\.]+",   # handles C#, C++, .NET, etc.
            ngram_range=(1, 2),                    # unigrams + bigrams
            lowercase=True
        )
        tfidf_matrix = vectorizer.fit_transform(corpus)          # shape (2, vocab)
...
def calculate_match_score(user_skills: str, user_experience: int,
    ...
    Algorithm:
      - Skill Score  : TF-IDF + Cosine Similarity (ML / NLP)
      - Exp Score    : Linear ratio  min(user_exp / required_exp, 1.0)
      - Overall      : 70 % skill score  +  30 % experience score
    """
    try:
        # ── 1. Parse raw skill lists (used for matched / missing display) ──────
        user_skills_list = [s.strip().lower() for s in user_skills.split(',') if s.strip()]
        job_skills_list  = [s.strip().lower() for s in job_skills.split(',')  if s.strip()]

        # ── 2. Exact-match lists for UI display ────────────────────────────────
        matched_skills = [s for s in user_skills_list if s in job_skills_list]
        missing_skills = [s for s in job_skills_list  if s not in user_skills_list]

        # ── 3. TF-IDF Cosine Similarity for skill score ────────────────────────
        if user_skills_list and job_skills_list:
            # Join skill lists back into sentences for TF-IDF
            user_doc = ' '.join(user_skills_list)
            job_doc  = ' '.join(job_skills_list)
            cosine_score = _tfidf_skill_similarity(user_doc, job_doc)   # [0, 1]
        else:
            cosine_score = 0.0

        skill_match_percentage = round(cosine_score * 100, 2)

        # ── 4. Experience match (linear ratio) ────────────────────────────────
        try:
            required_exp = int(str(job_experience).split()[0]) if job_experience else 0
        except (ValueError, IndexError):
            required_exp = 0

        if required_exp > 0:
            exp_ratio = min(user_experience / required_exp, 1.0)
            exp_match_percentage = round(exp_ratio * 100, 2)
        else:
            exp_match_percentage = 100.0

        # ── 5. Weighted overall score ──────────────────────────────────────────
        overall_match = round((skill_match_percentage * 0.7) + (exp_match_percentage * 0.3), 2)

        return {
            'overall_match'    : overall_match,
            'skill_match'      : skill_match_percentage,
            'exp_match'        : exp_match_percentage,
            'matched_skills'   : matched_skills,
            'missing_skills'   : missing_skills,
            'user_experience'  : user_experience,
            'required_experience': required_exp
